...
class DsqfBotApp:
    ...
    async def check_all_accounts_health(self, update: Update) -> None:
        ...
        progress_message = await self.render_message(
            update,
            self.all_accounts_health_progress_text(
                checked_accounts=0,
        ...
        for session_row in sessions:
            ...
            if status == "online":
                ...
                offline += 1
                note = refreshed_row.get("last_error") or "账号掉线"
                current_action = f"掉线 | {note}"
            elif status == "frozen":
                frozen += 1
                note = refreshed_row.get("last_error") or "账号已冻结"
                current_action = f"冻结 | {note}"
            else:
                other += 1
                note = refreshed_row.get("last_error") or self.human_session_status(status)
                current_action = f"异常 | {note}"

            summaries.append({"label": refreshed_row["label"], "status": status, "note": note})
            await self.edit_message(
                progress_message,
                self.all_accounts_health_progress_text(
                    checked_accounts=checked_accounts,
        ...
        final_text = self.all_accounts_health_done_text(summaries=summaries)
    ...
    async def on_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            if state == "wait_schedule_time":
                ...
                interval_minutes = self.batch_interval_minutes(payload["repeat_mode"])
    ...
    async def on_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            if data == "accounts":
                ...
                return
            if data == "accounts:check_health":
                await self.check_all_accounts_health(update)
                return
            if data == "account:add":
            ...
            if data.startswith("schedule:repeat:"):
                ...
                if interval_minutes is not None:
                    prompt = self.batch_schedule_prompt(repeat_mode, interval_minutes)
                else:
    ...
    def accounts_keyboard(self) -> InlineKeyboardMarkup:
        ...
        rows.append([InlineKeyboardButton("一键检查所有账号存活", callback_data="accounts:check_health")])
    ...
    def account_detail_keyboard(self, session_id: int) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup(
            [
                ...
                [InlineKeyboardButton("一键检查所有账号存活", callback_data="accounts:check_health")],
    ...
    def repeat_keyboard(self, allow_daily: bool) -> InlineKeyboardMarkup:
        ...
        if allow_daily:
            ...
            rows.append(
                [
                    InlineKeyboardButton("每天排满 5 分钟", callback_data="schedule:repeat:daily_interval:5"),
                    InlineKeyboardButton("每天排满 10 分钟", callback_data="schedule:repeat:daily_interval:10"),
                ]
            )
            rows.append(
                [
                    InlineKeyboardButton("每天排满 30 分钟", callback_data="schedule:repeat:daily_interval:30"),
                    InlineKeyboardButton("每天排满自定义", callback_data="schedule:repeat:daily_interval:custom"),
                ]
            )
        rows.append([InlineKeyboardButton("返回首页", callback_data="home")])
        return InlineKeyboardMarkup(rows)
    ...
    @staticmethod
    def batch_interval_minutes(repeat_mode: str) -> int | None:
        if not (repeat_mode.startswith("interval:") or repeat_mode.startswith("daily_interval:")):
            return None
        try:
    ...
    @staticmethod
    def is_daily_repeat_mode(repeat_mode: str) -> bool:
        return repeat_mode == "daily" or repeat_mode.startswith("daily_interval:")

    def batch_schedule_prompt(self, repeat_mode: str, interval_minutes: int) -> str:
    ...
    def repeat_mode_text(self, repeat_mode: str) -> str:
        ...
        if interval_minutes is not None:
            if self.is_daily_repeat_mode(repeat_mode):
                return f"每天重复（每 {interval_minutes} 分钟排满）"
    ...
    @staticmethod
    def all_accounts_health_progress_text(
        ...
        lines = [
            f"正在检查所有账号存活：{checked_accounts}/{total_accounts}",
            f"在线：{online}",
            f"掉线：{offline}",
            f"冻结：{frozen}",
            f"其他异常：{other}",
    ...
    def all_accounts_health_done_text(
        self,
        summaries: list[dict[str, Any]],
    ) -> str:
        ...
        other_count = max(0, len(summaries) - online_count - offline_count - frozen_count)
        lines = [
            "所有账号健康状态检查完成",
            f"账号数：{len(summaries)}",
            f"在线：{online_count} | 掉线：{offline_count} | 冻结：{frozen_count} | 其他异常：{other_count}",
            "",
            "账号明细：",
        ]
        for index, item in enumerate(summaries, start=1):
            detail = f"{index}. {item['label']} | {self.human_session_status(item['status'])}"
            if item.get("note"):
                detail += f" | {item['note']}"
            lines.append(detail)
        return "\n".join(lines)
